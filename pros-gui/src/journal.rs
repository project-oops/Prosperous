//! What this tool has done this session, and how each job turned out.
//!
//! This is not the log: the log is the target's own (`klogsrv`), and this records what this
//! program asked and what came back, kept apart so the two accounts are never confused. The
//! status bar shows only the current job; the journal keeps the sequence.
//!
//! One entry per job - what, when, how long, how it ended - never one per file transferred.
//! It lives for the session and is not written to disk: what a target could do earlier is not
//! a fact to read later, the same reason no capability is cached.
...
pub(crate) enum Ending {
    ...
    /// It did not work.
    Failed(String),
    /// It was declined before anything happened. A deliberate refusal, not a failure, and
    /// shown as such.
    Refused(String),
...
///
/// Bounds a long session; the oldest entries go first.
...
impl Journal {
    ...
    ///
    /// Finds the running entry rather than taking the last, which need not be the one that
    /// just finished.
...
mod tests {
    ...
    /// A job is recorded running, then with its result.
    #[test]
    ...
    /// A refusal is not counted as a failure.
    #[test]
    ...
    /// Failures are counted for the closed bar.
    #[test]
    ...
    /// Each result lands on the entry that was running.
    #[test]
    ...
    /// Clearing keeps running entries, so their results still have somewhere to land.
    #[test]
    ...
    /// At the bound, the oldest entries go first.
    #[test]
